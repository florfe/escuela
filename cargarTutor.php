...
require_once 'funciones/listar_vinculo.php'; 
$ListadoVinculoAlumnos=Listar_VinculoAlumnos($MiConexion);
$CantidadVinculoAlumnos=count($ListadoVinculoAlumnos);
...
require_once 'funciones/validar_registroTutor.php'; 
require_once 'funciones/insertar_tutor.php'; 
$Mensaje= '';
$Estilo= 'danger';
if(!empty($_POST['BotonRegistrar'])) {
$Mensaje= Validar_Datos();
if(empty($Mensaje)){
  if(InsertarTutor($MiConexion)!= false ) {
    $Mensaje= 'Tutor cargado!';
    $_POST= array();
   $Estilo='success';
}}}
 ...
 ?>
  ...
  <body class="app sidebar-mini">
    ...
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-edit"></i> Registrar Tutor</h1>
          <p>Completa todos los datos</p>
        </div>
        <ul class="app-breadcrumb breadcrumb">
          <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
          <li class="breadcrumb-item"><a href="principalalumnos.php">Inicio</a></li>
          <li class="breadcrumb-item"><a href="cargartutor.php">Registro</a></li>
        </ul>
      </div>
      <div class="row">
        <div class="col-md-12">
          <div class="tile">
            <h3 class="tile-title">Ingresa los datos personales del tutor<img  src= "icon/mother-and-daughter-playing-md-unscreen.gif" style="width:15%; position: absolute; right: 5%" /></h3>
...
<?php } ?>
      ...
      <i class="fa fa-asterisk" aria-hidden="true"></i>
          <select name="Vinculo" class="form-control form-control-user" id="Vinculo"style="width: 49%">
<option value="" >Selecciona...</option>
<?php for($i=0;$i<$CantidadVinculoAlumnos;$i++) {
if(!empty($_POST['Vinculo'])&& $_POST['Vinculo']==  $ListadoVinculoAlumnos[$i]['ID']){
  $selected='selected';}
  else{
    $selected='';
 }
    ?>
<option value="<?php echo $ListadoVinculoAlumnos[$i]['DENOMINACION'];  ?>" <?php echo $selected; ?> ><?php echo $ListadoVinculoAlumnos[$i]['DENOMINACION'] ?></option>
<?php } ?>
</select> 
      </div> 
                  </div>
   ...
   <input type="hidden" name="Edad" id="inputEdad"> 
    ...
    <!-- RENGLON DOS  TIPO Y  NUMERO  DNI-->
      ...
      <i class="fa fa-asterisk" aria-hidden="true"></i>
      <input type="text" class="form-control form-control-user" name="NumDni" id="NumDni" pattern="[0-9]{8}" minlength="8" maxlength="8">
      </div>
      </div>
    ...
    <input type="date" id="fechaVencimiento" class="form-control form-control-user" name="FechaVencimiento"  >
      ...
      <div class="col-sm-6">
      <label class="control-label">Cuil</label>  <i class="fa fa-asterisk" aria-hidden="true"></i>
      ...
      <div class="col-sm-6 "><label class="control-label">Numero </label> <i class="fa fa-asterisk" aria-hidden="true"></i> 
...
if(!empty($_POST['Localidad'])&& $_POST['Localidad']==  $ListadoLocalidad[$i]['ID']){
  $selected='selected';}
  else{
    $selected='';
 }
...
<?php } ?>
 ...
 <div class="col-sm-6 "><label class="control-label">Pais</label> 
  ...
  <div class="tile-footer">
    <button class="btn btn-primary" type="submit" name="BotonRegistrar" value="registrar"><i class="fa fa-fw fa-lg fa-check-circle"></i>Registrar</button>&nbsp;&nbsp;&nbsp;<a class="btn btn-secondary" href="cargarTutor.php"><i class="fa fa-fw fa-lg fa-times-circle"></i>Cancelar</a>
            </div>
            </div>
            </form>
          </div>
