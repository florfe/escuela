...
$materia = isset($_GET['materia']) ? urldecode($_GET['materia']) : "";
$variable=$_GET['id'];
...
nf.IdAlumnos, nf.Materias, nf.ColoqDiciembre

FROM alumnos al, notas pri, notassegunda seg, cursoscompletos cc, notasfinal nf
WHERE al.Id = pri.IdAlumnos AND al.Id = seg.IdAlumnos AND pri.Materias = seg.Materias and al.Id = nf.IdAlumnos and pri.Materias = nf.Materias
...
<select name="ColoquioFebrero[]"  id="coloquioFebrero" onchange="colorChange(this)"><option value=""></option>';
<?php for ($i = 0; $i < $CantidadNotas; $i++) {
  $selected = (!empty($_POST["ColoquioFebrero"]) && $_POST["ColoquioFebrero"] == $ListadoNotas[$i]["ID"]) ? 'selected' : '';
